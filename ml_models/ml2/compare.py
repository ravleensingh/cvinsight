...
def candidate_models() -> dict:
    return {
        ...
        "multinomial_naive_bayes": MultinomialNB(alpha=0.2),
        ...
        "decision_tree": DecisionTreeClassifier(max_depth=70, class_weight="balanced", random_state=42),
        "random_forest": RandomForestClassifier(n_estimators=250, max_depth=90, min_samples_leaf=2, class_weight="balanced", random_state=42, n_jobs=-1),
    }
...
def plot_confusion(y_test, y_pred, classes):
    ...
    plt.title("ML2 Confusion Matrix")
...
def write_model_card(metrics: dict):
    content = f"""# ML2 Structured Resume Category Classifier

- Dataset: C0ldSmi1e/resume-dataset
- Task: Resume category classification with text, skills, education, and experience
...
ML2 validates how much structured resume fields improve category prediction compared with raw text alone.
"""
...
def compare_algorithms() -> dict:
    ...
    metrics.update(
        {
            "model_id": MODEL_ID,
            "display_name": "ML2 Structured Resume Category Classifier",
            "task_type": "resume_category_classification",
            "dataset_name": "C0ldSmi1e/resume-dataset",
    ...
    update_registry(
        {
            ...
            "algorithm": best_name,
            "dataset_name": "C0ldSmi1e/resume-dataset",
            "artifact_path": "artifacts/ml2/model.joblib",
            "metrics_path": "artifacts/ml2/metrics.json",
            "classes": [str(label) for label in classes],
        }
    )
    print(results.to_string(index=False))
    print(f"ML2 selected: {best_name}")
    return metrics
